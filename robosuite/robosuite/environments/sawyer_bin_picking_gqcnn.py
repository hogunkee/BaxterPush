...
from robosuite.gqcnn.policy_mujoco import evaluate_gqcnn

from matplotlib import pyplot as plt


class SawyerBinPickingGQCNN(SawyerEnv):
    def __init__(
        self,
    ...
    def _load_model(self):
        ...
        self.ob_inits = [CanObject]*1 #, CanObject, CanObject, CanObject]
        self.vis_inits = [
            MilkVisualObject,
            BreadVisualObject,
            CerealVisualObject,
        ...
        self.item_names = ["Milk"+str(i) for i in range(1)] # , "Can2", "Can3", "Can4"]
    ...
    def gqcnn(self):
        ...
        if self.use_camera_obs:
            ...
            grasp = evaluate_gqcnn(np.flip(di["image"],axis=0), np.flip(di["depth"],axis=0), normalize=True) #, vis_on=False)
    ...
    def _get_observation(self):
        ...
        if self.use_camera_obs:
            ...
            '''
            if self.run_gqcnn == True:
                #plt.imshow(di["image"])
                #plt.show()
                grasp = evaluate_gqcnn(di["image"], di["depth"], normalize=True, vis_on=False)
                self.run_gqcnn = False
            '''
    ...
    def _pixel2pos(self, X, Y, depth):
        """
        x1, x2, y1, y2 = 0.7-0.2, 0.7+0.2, -0.15-0.25, -0.15+0.25# 0.6, 0.8, -0.275, -0.025
        X1, X2, Y1, Y2 = 42, 214, 19, 236
        ...
        return x,y
        """
        fovy = self.sim.model.cam_fovy[0]
        h, w = 256., 256. 
        ...
        x *= depth # 0.52/z
        y *= depth # 0.52/z
        #x = -1.0 * np.tan(np.pi*(X-128)/1024)
        #y = -1.0 * np.tan(np.pi*(Y-128)/1024)
        return -x+0.7, -y-0.15


class SawyerBinPickingSingle(SawyerBinPickingGQCNN):
    """
...
class SawyerBinPickingMilk(SawyerBinPickingGQCNN):
    """
...
class SawyerBinPickingBread(SawyerBinPickingGQCNN):
    """
...
class SawyerBinPickingCereal(SawyerBinPickingGQCNN):
    """
...
class SawyerBinPickingCan(SawyerBinPickingGQCNN):
    """
