...
class BaxterActiveSensing(BaxterEnv):
    def __init__(
        self,
        ...
        # load goal images
        data = np.load('/home/cyh/robosuite-gqcnn/robosuite/robosuite/scripts/template.npz')
        self.g_rgb = data['rgb']
        info = np.iinfo(self.g_rgb.dtype)
        self.g_rgb = cv2.resize(self.g_rgb,(64,64))
        self.g_rgb = self.g_rgb.astype(np.float) / info.max
        self.g_d = data['d']
    ...
    def _load_model(self):
        ...
        obj_list = [LObject] #, TObject] #, UObject]
        ...
        #self.ob_inits = [CanObject]*3 #, CanObject, CanObject, CanObject]
        self.vis_inits = []
        # self.vis_inits = [
        #     MilkVisualObject,
        #     BreadVisualObject,
    ...
    def _reward(self, rgb=None, action=None):
        # rgb, d = self.show_image()
        # rgb = cv2.resize(rgb,(64,64))
        # rgb = rgb.astype(np.float) / 255
        # print(np.shape(self.g_rgb))
        # print(np.shape(rgb))
        reward = -np.linalg.norm(self.g_rgb-rgb)**2 / 1000
        # reward -= -0.01
    ...
    def show_image(self):
        ...
        di["depth"] = near / (1 - di["depth"] * (1 - near / far))

        while np.min(di["depth"])>0.99:
            camera_obs = self.sim.render(
                camera_name=self.camera_name,
                width=self.camera_width,
                height=self.camera_height,
