...
from robosuite.models.objects import (
    ...
    obj1,
    obj2,
    BlueSaltCube, Rusk, FlowerCup, Glassbowl, GreenCup, GreenSaltCylinder, RedCup, SmallGlass,
                    Waterglass, YellowSaltCube, 
                    CokePlasticSmallGrasp, LivioClassicOil, Shampoo, ShowerGel, InstantSoup, Toothpaste
)
from robosuite.models.objects import (
    MilkVisualObject,
    BreadVisualObject,
...
class BaxterCollectData(BaxterEnv):
    def __init__(
        self,
        gripper_right="TwoFingerGripper",
        gripper_left="LeftTwoFingerGripper",
        table_full_size=(0.39, 0.49, 5), #0.82),
    ...
    def _load_model(self):
        ...
        # The baxter robot has a pedestal, we want to align it with the table
        self.mujoco_arena.set_origin([0.5, -0.4, 0])   #.5 -.3
        ...
        obj_list = [BlueSaltCube, FlowerCup, Glassbowl, GreenCup, GreenSaltCylinder, RedCup, SmallGlass,
                    Waterglass, YellowSaltCube, CokePlasticSmallGrasp, LivioClassicOil, Shampoo, ShowerGel,  Toothpaste] # InstantSoup, TObject] #, UObject]
        # choice = np.random.choice(np.arange(14),num_objects)
        # k = 0
        # while os.path.exists('/home/cyh/robosuite-gqcnn/robosuite/robosuite/scripts/multi_obs_dataset/'+str(num_objects)+'/choice'+str(k)+'.npy'):
        #     k+=1
        # np.save('/home/cyh/robosuite-gqcnn/robosuite/robosuite/scripts/multi_obs_dataset/'+str(num_objects)+'/choice'+str(k)+'.npy', choice)
        
        self.scene += 1
        print('Scene Number: ', self.scene)
        choice = np.load('/home/rllab/robosuite-gqcnn/robosuite/robosuite/scripts/multi_obs_dataset/'+str(num_objects)+'/choice'+str(self.scene)+'.npy')

        self.ob_inits = (np.array(obj_list)[choice.astype(int)]).tolist()
        # self.ob_inits = np.random.choice(obj_list,num_objects)
        print(self.ob_inits)
        # for i in range(num_objects):
        #     self.ob_inits.append(obj_listnp.random.choice())
        # self.ob_inits = [LObject]*10  # ConcaveObject
        # self.ob_inits = [Glassbowl]*10 #, CanObject, CanObject, CanObject]
        self.vis_inits = []
        # self.vis_inits = [
        #     MilkVisualObject,
        #     BreadVisualObject,
        ...
        self.item_names = ["Can"+str(i) for i in range(num_objects)] # , "Can2", "Can3", "Can4"]
        ...
        self.model.place_objects(self.scene)
    ...
    def show_image(self):
        ...
        if self.camera_depth:
            ...
            di["image"] = camera_obs
        plt.imshow(np.flip(di["depth"],axis=0))
        plt.show()


    def gqcnn(self, vis_on=True, num_candidates=1):
        di = super()._get_observation()
        ...
        # print(ddd)
        # print(di["depth"])
        print(np.min(di["depth"]), np.max(di["depth"]))
        ...
        '''
        return evaluate_gqcnn(np.flip(di["image"],axis=0), np.flip(di["depth"],axis=0), \
                normalize=False, vis_on=vis_on, model_name=model_name, crop_size=crop_size)
        '''
