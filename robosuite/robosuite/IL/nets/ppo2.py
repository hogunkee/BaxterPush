...
class Agent:
    def __init__(self, env, arg_parser, agent_id):
        ...
        with tf.variable_scope(self.model_name):
            ...
            self.disc_targets = tf.placeholder(tf.float32, [None], name='disc_returns')
            self.const_targets = tf.placeholder(tf.float32, [None], name='const_returns')

            #model 짜기
            self.mean, self.std = self.build_policy_model('policy')
            self.disc_value = self.build_value_model('disc_value')
            self.const_value = self.build_value_model('const_value')
            ...
            #train 모델 짜기
            self.train_op, self.disc_value_train_op, self.const_value_train_op = self.build_op()
    ...
    def build_op(self):
        ...
        loss_disc_value = 0.5*tf.square(self.disc_targets - self.disc_value)
        self.loss_disc_value = tf.reduce_mean(loss_disc_value)

        train_vars2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tf.get_variable_scope().name+'/disc_value')
        optimizer2 = tf.train.AdamOptimizer(learning_rate=self.value_lr)
        train_op2 = optimizer2.minimize(self.loss_disc_value, var_list= train_vars2)
            
        loss_const_value = 0.5*tf.square(self.const_targets - self.const_value)
        self.loss_const_value = tf.reduce_mean(loss_const_value)

        train_vars3 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tf.get_variable_scope().name+'/const_value')
        optimizer3 = tf.train.AdamOptimizer(learning_rate=self.value_lr)
        train_op3 = optimizer3.minimize(self.loss_const_value, var_list= train_vars3)

        return train_op, train_op2, train_op3
    ...
    def get_action(self, state, is_train):
        ...
        [disc_value], [const_value] = self.sess.run([self.disc_value, self.const_value], feed_dict={self.states: [state]})
        return action, disc_value, const_value
    ...
    def train(self, inp, batch_size=64, only_value=False): # TRAIN POLICY
        ...
        num_batches = max(states.shape[0] // batch_size, 1)
        batch_size = states.shape[0] // num_batches

        for _ in range(self.value_epochs):
            states, actions, gaes, disc_targets, const_targets, old_means, old_stds = shuffle(states, actions, gaes, disc_targets, const_targets, old_means, old_stds, random_state=0)
            for j in range(num_batches): 
                start = j * batch_size
                end = start + batch_size
                self.sess.run([self.disc_value_train_op, self.const_value_train_op], \
                    ...
                    self.disc_targets:disc_targets[start:end], \
                    self.const_targets:const_targets[start:end]})
        ...
        loss_p, loss_disc_v, loss_const_v, kl, entropy = self.sess.run([self.loss_policy, self.loss_disc_value, self.loss_const_value, self.kl, self.entropy], \
                    ...
                    self.const_targets:const_targets})

        return loss_p, loss_disc_v, loss_const_v, kl, entropy
